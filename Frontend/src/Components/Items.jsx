...
const Items = (props) => {
  ...
  return (
    <Link to={`/${currentCategory}/${props.id}`} className="block">
      <div className="group tablet:w-40 laptop:max-w-[300px] laptop:w-full relative cursor-pointer rounded-2xl border border-slate-100 bg-white p-4 shadow-md ring-1 ring-transparent transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:ring-blue-200/70">
        ...
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-b from-slate-50 to-slate-100 py-2 ring-1 ring-slate-200/60 ring-inset">
          ...
          {/* Discount badge */}
          {hasDiscount && (
            <span className="absolute top-2 left-2 rounded-full bg-gradient-to-r from-rose-500 to-rose-600 px-2.5 py-1 text-[11px] font-bold text-white shadow-md shadow-rose-200">
              -{discountPercent}%
            </span>
          )}
        ...
        <div className="space-y-2.5 pt-4">
          <p className="laptop:text-base truncate text-sm font-medium text-slate-700 transition-colors duration-200 group-hover:text-slate-900">
            {props.name}
          </p>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-baseline gap-1.5">
              ...
              {hasDiscount && (
                <span className="font-productSansLight text-xs text-slate-400 line-through">
            ...
            {/* Add-to-cart button */}
            <button className="inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-full bg-gradient-to-r from-[#2b2a28] via-[#3a3937] to-[#222220] px-3.5 py-1.5 text-xs font-semibold text-white shadow-sm transition-all duration-200 hover:scale-105 hover:shadow-md active:scale-95">
              <FaCartPlus className="h-3.5 w-3.5" />
              <span className="hidden laptop:inline">Add to cart</span>
              <span className="laptop:hidden">Add</span>
            </button>
          </div>
        </div>
